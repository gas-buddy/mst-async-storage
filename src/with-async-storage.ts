import { applySnapshot, IStateTreeNode, onSnapshot, getSnapshot, getType } from "mobx-state-tree"
...
export interface WithAsyncStorageOptions {
   ...
   */
  onLoad?: (loaded: object | undefined) => object | undefined | {}

  /**
   * A function that will be called after a snapshot was taken
   * but before it is passed to AsyncStorage
   */
  onSave?: (snapshot: object) => object | undefined
}

/**
 * Adds AsyncStorage support to your model.
 */
export const withAsyncStorage =
  (options: WithAsyncStorageOptions = {}) =>
  (self: IStateTreeNode) => {
    ...
    return {
      actions: {
        ...
        /**
         * Saves the snapshot to mmkv storage. This only needs to be
         * called if autoSave has been turned off.
         */
        save() {
          let snapshot = filterSnapshotKeys(getSnapshot(self))
          if (options.onSave) {
            snapshot = options.onSave(snapshot)
          }
          save(key, snapshot)
        },
